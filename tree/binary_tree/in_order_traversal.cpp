...
// Function for performing in-order traversal
void in_order(Node *node)
{
    // First perfrom in_order traversal to the left node
    if(node->left != NULL){
        in_order(node->left);
    }

    // visit or print the root node of it's subtree.
    cout << node->data << " ";

    // Then perform in_order traversal to the right node
    if(node->right != NULL){
        in_order(node->right);
    }
...
{   
    ...
    cout << "Now perfroming in-order traversal of the tree : ";
    in_order(root);
    cout << endl;
