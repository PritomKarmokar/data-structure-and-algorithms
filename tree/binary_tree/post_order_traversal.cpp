...
// Function for performing post-order traversal
void post_order(Node *node)
{
    // First perform post_order traversal to the left node
    if(node->left != NULL){
        post_order(node->left);
    }

    // Then perfrom post_order traversal to the right node
    if(node->right != NULL){
...
{   
    ...
    cout << "Now perfroming post-order traversal of the tree : ";
    post_order(root);
    cout << endl;
